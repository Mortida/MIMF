...
def cmd_client_inspect(args: argparse.Namespace) -> int:
    ...
    Time:  O(n) client-side read + network
    Space: O(n) request body (bounded)
    """
    c = MIMFHttpClient(args.url, api_key=args.api_key, max_upload_bytes=args.max_upload_bytes)
    fields = {}
    if args.labels:
        fields['labels'] = args.labels
    r = c.post_multipart('/inspect', fields=fields, file_field=('file', os.path.basename(args.file)), file_path=args.file)
    if r.status >= 400:
        print(r.body_bytes.decode('utf-8', errors='replace'), file=sys.stderr)
        return 2
    _print_json(r.json())
...
def cmd_client_normalize(args: argparse.Namespace) -> int:
    ...
    if args.labels:
        fields['labels'] = args.labels
    r = c.post_multipart('/normalize', fields=fields, file_field=('file', os.path.basename(args.file)), file_path=args.file)
    if r.status >= 400:
        print(r.body_bytes.decode('utf-8', errors='replace'), file=sys.stderr)
        return 2
    _print_json(r.json())
...
def cmd_client_export_bundle(args: argparse.Namespace) -> int:
    ...
    r = c.post_multipart('/export-bundle', fields=fields, file_field=('file', os.path.basename(args.file)), file_path=args.file)
    if r.status >= 400:
        print(r.body_bytes.decode('utf-8', errors='replace'), file=sys.stderr)
        return 2
...
def cmd_demo_api(args: argparse.Namespace) -> int:
    ...
    # Inspect
    r1 = c.post_multipart('/inspect', fields={'labels': args.labels or ''}, file_field=('file', os.path.basename(args.file)), file_path=args.file)
    if r1.status >= 400:
    ...
    out = {
        ...
        'export': {
            ...
            'context_id': r3.headers.get('X-MIMF-Context-Id') or r3.headers.get('x-mimf-context-id'),
            'bundle_id': r3.headers.get('X-MIMF-Bundle-Id') or r3.headers.get('x-mimf-bundle-id'),
            'merkle_root': r3.headers.get('X-MIMF-Merkle-Root') or r3.headers.get('x-mimf-merkle-root'),
        },
        'verify': r4.json(),
    }
    _print_json(out)
...
def register_client_commands(sub: argparse._SubParsersAction) -> None:
    ...
    client.add_argument('--max-upload-bytes', type=int, default=25*1024*1024, help='Client-side upload cap')
    ...
    norm.add_argument('--boundary-caps', default='export:document.basic', help='Comma-separated boundary caps (hint)')
    ...
    exp.add_argument('--include-original', action='store_true', help='Include original bytes in bundle')
    exp.add_argument('--persist', action='store_true', help='Persist context in server DB (requires runtime:write)')
    exp.add_argument('--boundary-id', default='api-export', help='Boundary id')
    exp.add_argument('--boundary-caps', default='export:document.basic', help='Comma-separated boundary caps (hint)')
    ...
    vb.add_argument('--public-key', default=None, help='Path to Ed25519 public key for bundle signature')
    ...
    vb.add_argument('--receiver-public-key', default=None, help='Receiver pubkey for transfer receipts')
    ...
    demo.add_argument('--include-original', action='store_true', help='Include original bytes in bundle')
    demo.add_argument('--persist', action='store_true', help='Persist context in server DB (requires runtime:write)')
    demo.add_argument('--boundary-id', default='api-export', help='Boundary id')
    demo.add_argument('--boundary-caps', default='export:document.basic', help='Comma-separated boundary caps (hint)')
    demo.add_argument('--strict', action='store_true', help='Deny instead of redact')
    demo.add_argument('--max-upload-bytes', type=int, default=25*1024*1024, help='Client-side upload cap')
    ...
    demo.add_argument('--receiver-public-key', default=None, help='Receiver receipt pubkey (optional)')
