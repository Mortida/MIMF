...
class MIMFHttpClient:
    ...
    def __init__(self, base_url: str, api_key: Optional[str] = None, max_upload_bytes: int = 25 * 1024 * 1024):
    ...
    def post_multipart(
        ...
        *,
        ...
        Time:  O(n) to build request body (bounded by max_upload_bytes)
        Space: O(n) request body (in-memory)
...
def _do_request(req: Request) -> HttpResponse:
    ...
    try:
        ...
        return HttpResponse(status=int(getattr(e, 'code', 0) or 0), headers=headers, body_bytes=body)
